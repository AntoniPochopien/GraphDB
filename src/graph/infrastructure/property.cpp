...
namespace graphdb
{
    void PropertyValue::serialize(ofstream& out) const
    {
        std::visit([&out](auto&& arg){
            ...
            } else if constexpr (std::is_same_v<T,shared_ptr<PropertyMap>>) {
    ...
    PropertyValue PropertyValue::deserialize(ifstream& in)
    {
        ...
        switch(type)
        {
            case 0: { int val; in.read(reinterpret_cast<char*>(&val), sizeof(val)); return PropertyValue(val); }
            case 1: { double val; in.read(reinterpret_cast<char*>(&val), sizeof(val)); return PropertyValue(val); }
            case 2: { bool val; in.read(reinterpret_cast<char*>(&val), sizeof(val)); return PropertyValue(val); }
            case 3: {
                size_t len; in.read(reinterpret_cast<char*>(&len), sizeof(len));
                string s(len, '\0'); in.read(&s[0], len);
            ...
            case 4: {
                ...
                for (size_t i=0;i<count;++i){
                    size_t klen; in.read(reinterpret_cast<char*>(&klen), sizeof(klen));
                    string k(klen,'\0'); in.read(&k[0], klen);
